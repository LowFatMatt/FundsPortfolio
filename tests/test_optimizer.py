...
class TestPortfolioOptimizer:
    ...
    def test_determine_risk_level(self):
        ...
        assert opt._determine_risk_level({"risk_approach": "conservative"}) == (
            1,
            False,
        )
    ...
    def test_conservative_allocation(self, sample_funds):
        opt = PortfolioOptimizer()
        recs, meta = opt.optimize_portfolio(
            {"risk_approach": "conservative"}, sample_funds
        )
    ...
    def test_aggressive_allocation(self, sample_funds):
        opt = PortfolioOptimizer()
        recs, meta = opt.optimize_portfolio(
            {"risk_approach": "aggressive"}, sample_funds
        )
