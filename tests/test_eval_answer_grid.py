...
def test_full_grid_count_matches_combinatorics():
    grid = build_answer_grid()
    # 3 risk x 3 esg x 3 etf x (2^5 region subsets) x
    # (C(11,0)+C(11,1)+C(11,2) theme subsets)
...
def test_all_region_and_theme_cardinalities_present():
    ...
    assert region_sizes == set(range(0, len(REGIONS) + 1))  # 0..5
