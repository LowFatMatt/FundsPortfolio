...
| Feasibility advisor v2 | `funds_portfolio/dialog/feasibility.py` | Answer-space shaping: L0 option exclusions (`option_exclusions_by_profile` — `etf_only` not offered under DEF/BAL, stale values downgraded via `option_fallbacks`), L2 availability gating of themes+regions by risk band ∧ ESG-only ∧ ETF-only filters, and a shared selection budget DEF 1 / BAL 2 / OPP 3 with BALANCED per-field composition caps 1 region + 1 theme (L1, `per_field_max_by_profile`); pure functions, soft warnings for direct API calls |
