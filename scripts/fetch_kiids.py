...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/kiid_retriever.log'),
        ...
        for i, isin in enumerate(isins, 1):
            ...
            if status == 'success':
                ...
                if status2 == 'success':
                    ...
                    logger.info(f"  ✓ Found (Vanguard)")
                else:
                    results['pending'].append({
                        'isin': isin,
                        'reason': 'Search redirect not found (manual review needed)'
                    })
                    logger.warning(f"  ⚠ Pending manual review")
            
            else:
                results['failed'].append({
                    'isin': isin,
                    'error': f'iShares search: {status}'
                })
...
def generate_report(results: Dict, output_dir: str):
    ...
    with open(md_file, 'w') as f:
        f.write(f"# KIID Retrieval QC Checklist\n\n")
        ...
        if results['pending']:
            f.write(f"## ISINs for Manual Review\n\n")
            f.write(f"These ISINs need manual KIID lookup:\n\n")
            for item in results['pending']:
                f.write(f"- [ ] **{item['isin']}**: {item['reason']}\n")
            f.write(f"\n**Action:** Search iShares/Vanguard website directly, verify KIID URL, update `funds_database.json`\n\n")
        
        if results['failed']:
            f.write(f"## Failed ISINs (Investigate)\n\n")
    ...
    with open(csv_file, 'w') as f:
        ...
        for item in results['verified']:
            f.write(f"{item['isin']},{item['kiid_url']},{item['provider']},verified,Retrieved automatically\n")
...
def merge_into_funddb(verified_results: List[Dict], funddb_path: str):
    ...
    for item in verified_results:
        if item['isin'] in isin_lookup:
            ...
            isin_lookup[item['isin']]['kiid_retrieved_at'] = datetime.utcnow().isoformat()
            updated_count += 1
    
    # Save backup
    backup_path = funddb_path.replace('.json', f'_backup_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}.json')
...
def main():
    ...
    parser.add_argument('--sample', type=int, default=None, help='Test on first N ISINs only')
    parser.add_argument('--output', default='reports/', help='Output directory for reports')
    parser.add_argument('--merge', action='store_true', help='Auto-merge verified KIIDs into funds_database.json')
    parser.add_argument('--funddb', default='funds_database.json', help='Path to funds database')
    ...
    if args.merge and results['verified']:
        confirm = input(f"\nMerge {len(results['verified'])} verified KIIDs into {args.funddb}? (y/n): ")
        if confirm.lower() == 'y':
