...
from _german_labels import (  # noqa: E402  (import after sys.path setup)
...
def _best_horizon(
    d: Optional[Dict[str, Any]], order: Tuple[str, ...] = ("3y", "5y", "1y")
) -> Optional[float]:
...
def build_record(
    ...
    volatility = (
        round(ts_vol * 100.0, 4) if ts_vol is not None else gp.get("volatility")
    )

    ts_mdd = _best_horizon(ts_rm.get("max_drawdown"))
    max_drawdown = (
        round(abs(ts_mdd) * 100.0, 4) if ts_mdd is not None else gp.get("max_drawdown")
    )
    ...
    region = (
        ts_region if ts_region and ts_region != "global" else gp.get("region", "global")
    )
    ...
    record: Dict[str, Any] = {
        ...
        "notes": gp.get("notes")
        or "Imported from customer universe; enriched from per-ISIN scrape + general profile.",
        "source": "provinzial_nord_universe",
        "asset_class_breakdown": ts.get("asset_class_breakdown")
        or gp.get("asset_class_breakdown"),
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--customer", required=True, help="Customer profile id (e.g. provinzial_nord)"
    )
    parser.add_argument(
        "--source",
        required=True,
    ...
    output_path = args.output or (
        REPO_ROOT / "data" / "customers" / args.customer / "funds_database.json"
    )
    catalog = {
        ...
        "metadata": {
            ...
            "source_file": str(
                args.source.relative_to(REPO_ROOT)
