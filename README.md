...
5. **Dialog feasibility gating** ([funds_portfolio/dialog/feasibility.py](funds_portfolio/dialog/feasibility.py)) — the questionnaire only offers what the engine can honor: theme/region options with zero funds under the answered risk band ∧ ESG-only ∧ ETF-only filters render disabled-with-reason, and a shared selection budget (DEFENSIVE 1 / BALANCED 2 / OPPORTUNITY 3 combined regions+themes; BALANCED additionally capped at 1 region + 1 theme via `per_field_max_by_profile`) caps preferences. `etf_only` itself is excluded under DEFENSIVE/BALANCED via `option_exclusions_by_profile` (their ETF-only universe is too small — user-journey decision D-19). Infeasible direct-API answers are still accepted but logged as soft warnings.
