...
class Portfolio:
    ...
    def set_risk_profile(self, risk_profile: Optional[str]) -> None:
        self.risk_profile = risk_profile
        self.updated_at = self._get_iso_timestamp()

    def set_portfolio_metrics(self, metrics: Dict[str, Any]) -> None:
        self.portfolio_metrics = metrics
        self.updated_at = self._get_iso_timestamp()

    def set_explanations(self, explanations: Dict[str, Any]) -> None:
        self.explanations = explanations
        self.updated_at = self._get_iso_timestamp()

    def set_decision_trace(self, trace: Dict[str, Any]) -> None:
    ...
    def add_recommendation(
        self, isin: str, name: str, allocation_percent: float, rationale: str
    ) -> None:
        """
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "portfolio_metrics": self.portfolio_metrics or self.calculated_metrics,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Portfolio":
        ...
        portfolio.risk_profile = data.get("risk_profile")
        portfolio.portfolio_metrics = data.get("portfolio_metrics", {})
        portfolio.explanations = data.get("explanations", {})
        portfolio.decision_trace = data.get("decision_trace", {})
        return portfolio
    ...
    def validate(self) -> tuple[bool, List[str]]:
        ...
        if self.recommendations:
            total_allocation = sum(
                r.get("allocation_percent", 0) for r in self.recommendations
            )
            if not (99.5 <= total_allocation <= 100.5):
                errors.append(
                    "Recommendation allocations must sum to ~100% "
                    f"(got {total_allocation}%)"
                )
