...
class PortfolioOptimizer:
    ...
    def _determine_risk_level(self, user_answers: Dict) -> Tuple[int, bool]:
        ...
        if approach == 'conservative' or approach == '1': return 1, False
        if approach == 'moderate_low' or approach == '2': return 2, False
        if approach == 'moderate' or approach == '3': return 3, False
        if approach == 'aggressive' or approach == '4': return 4, False
        
        # Default fallback
        logger.warning("Unrecognized risk approach '%s', defaulting to 2 (moderate low)", approach)
        return 2, True

    def optimize_portfolio(self, user_answers: Dict, enriched_funds: List[Dict]) -> Tuple[List[Dict], Dict]:
        """
