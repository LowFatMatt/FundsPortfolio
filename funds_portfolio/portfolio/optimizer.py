...
class PortfolioOptimizer:
    ...
    def __init__(self):
        # Target equity allocation based on risk level 1 (Conservative) to 4 (Aggressive)
    ...
    def _determine_risk_level(self, user_answers: Dict) -> int:
        """
        ...
        if approach == 'conservative' or approach == '1': return 1
        if approach == 'moderate_low' or approach == '2': return 2
        if approach == 'moderate' or approach == '3': return 3
        if approach == 'aggressive' or approach == '4': return 4
        
        # Default fallback
        logger.warning("Unrecognized risk approach '%s', defaulting to 2 (moderate low)", approach)
        return 2

    def optimize_portfolio(self, user_answers: Dict, enriched_funds: List[Dict]) -> List[Dict]:
        """
        ...
        risk_level = self._determine_risk_level(user_answers)
        target_allocation = self.risk_allocations[risk_level]
        
        # Separate into equity and bonds (treating everything else as bond/conservative for MVP)
        equities = [f for f in enriched_funds if f.get('asset_class', '').lower() == 'equity']
        bonds = [f for f in enriched_funds if f.get('asset_class', '').lower() != 'equity']
        ...
        if equities and equity_share > 0:
            ...
            for f in top_equities:
                ...
                rec['rationale'] = f'Top Sharpe Equity component (Risk level {risk_level})'
        ...
        if bonds and bond_share > 0:
            ...
            for f in top_bonds:
                rec = dict(f)
                rec['allocation_percent'] = round(per_fund_share, 2)
                rec['rationale'] = f'Top Sharpe Fixed Income component (Risk level {risk_level})'
                recommendations.append(rec)
        ...
        if recommendations:
            ...
            if abs(diff) > 0.001:
                ...
                recommendations[0]['allocation_percent'] = round(recommendations[0]['allocation_percent'] + diff, 2)
