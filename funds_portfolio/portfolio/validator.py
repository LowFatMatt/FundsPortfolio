...
class PortfolioValidator:
    ...
    def validate_recommendations(self, recommendations: List[Dict]) -> Tuple[bool, List[str]]:
        """
        Check if the proposed allocations meet rules.
        
        Args:
            recommendations: List of dictionaries with 'allocation_percent', 'yearly_fee', etc.
        ...
        if len(recommendations) < self.min_funds:
            errors.append(f"Portfolio lacks diversification: {len(recommendations)} funds (minimum {self.min_funds})")
            
        total_allocation = sum(r.get('allocation_percent', 0.0) for r in recommendations)
        ...
        if weighted_fee > self.max_fee:
            errors.append(f"Expected fee is {weighted_fee:.2f}%, exceeding maximum {self.max_fee:.2f}%")
